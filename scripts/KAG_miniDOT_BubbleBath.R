...
# Calculate oxygen saturation based on temp, do mg/L and elevation of 
    miniDot_data$do_sat <- DO.saturation

# Trim data to only include 
    miniDot_data_trimmed <- miniDot_data[miniDot_data$date_time >= as.POSIXct("2025-06-06 10:15:00") , ] # Trim to only after the sensors were placed in the bath 
    miniDot_data_trimmed <- miniDot_data[miniDot_data$date_time <= as.POSIXct("2025-06-06 16:15:00") , ] # Trim to only before sensors where removed from bath 
